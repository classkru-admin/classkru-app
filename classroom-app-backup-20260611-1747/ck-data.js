...
function getStudentsForPeriod(periodId){
  ...
  return students.map(s=>{
    ...
    // ถ้ายังไม่มี period attendance ใช้ history เดิม (backward compat)
    const hasNewData = Object.keys(periodHistory).length > 0;
    return {...s, history: hasNewData ? periodHistory : s.history, periodId};
  });
}
...
function getAttendanceSummaryForPeriod(periodId, dateKey){
  ...
  const records = getAttendance(periodId, dateKey);
  const c = {present:0,late:0,absent:0,leave:0,total:students.length};
  const hasNewData = Object.keys(records).length > 0;
  students.forEach(s=>{
    const r = hasNewData ? records[s.id] : s.history[dateKey];
    if(r && c[r.status]!==undefined) c[r.status]++;
  });
  return c;
}
